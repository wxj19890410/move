...
public class HtChart {
	...
	/*	QueryUtils.addWhere(qb, "and t.month like {0}", timeYear + "%");*/
		...
		int size = null!=dataResults?dataResults.size():0;
		List<BigDecimal> value11s = this.setListData(size);
		List<BigDecimal> value12s = this.setListData(size);
		List<BigDecimal> value13s = this.setListData(size);
		List<BigDecimal> value14s = this.setListData(size);
		List<BigDecimal> value15s = this.setListData(size);
		List<BigDecimal> value16s = this.setListData(size);
		int t = 0;
	...
	private List<BigDecimal> setListData(int size) {
	...
	public Object findDeptRank(String month, UserInfo userInfo) {
		...
		QueryUtils.addColumn(qb,
				"(select count(t1.id)+1 from DataOriginal t1 where t1.month =t.month and t1.total > t.total and not exists(from DeptRelation u where u.deptId = t1.deptId and u.deptType = '02'))",
				"rank");
		QueryUtils.addColumn(qb, "t.name", "name");
		...
		QueryUtils.addWhere(qb,
				"and (select count(t1.id) from DataOriginal t1 where t1.month =t.month and t1.total > t.total and not exists(from DeptRelation u where u.deptId = t1.deptId and u.deptType = '02')) < 10");
		...
		QueryUtils.addColumn(qb, "t.name", "name");
		QueryUtils.addColumn(qb,
				"(select count(t1.id)+1 from DataOriginal t1 where t1.month =t.month and t1.total > t.total and exists(from DeptRelation u where u.deptId = t1.deptId and u.deptType = '02'))",
				"rank");
		QueryUtils.addColumn(qb, "t.value1", "study");
		QueryUtils.addColumn(qb, "t.value2", "read");
		QueryUtils.addColumn(qb, "t.value3", "culture");
		...
		QueryUtils.addWhere(qb,
				"and (select count(t1.id) from DataOriginal t1 where t1.month =t.month and t1.total > t.total and exists(from DeptRelation u where u.deptId = t1.deptId and u.deptType = '02')) < 10");
		if (StringUtils.isNotBlank(month)) {
			QueryUtils.addWhereIfNotNull(qb, "and t.month = {0}", month);
		}
		QueryUtils.addOrder(qb, "t.total desc");
	...
	public Object getTagData(String timeYear, Integer tagid, UserInfo userInfo) {
		Map<String, Object> data = new HashMap<>();
		List<String>seriesX =Lists.newArrayList();
		List<Map<String, Object>> data1 = Lists.newArrayList();
		List<Map<String, Object>> data2 = Lists.newArrayList();
		QueryBuilder qb = new QueryBuilder();
		...
		/*QueryUtils.addWhere(qb, "and t.month like {0}", "%" + timeYear + "%");*/
		QueryUtils.addOrder(qb, "t.month");
		QueryUtils.addOrder(qb, "t.createDate desc");
		List<DataResult> dataResults = dataService.resultList(qb);
		...
		if (null != dataResults && dataResults.size() > 0) {
			for (DataResult dataResult : dataResults) {
				seriesX.add(dataResult.getMonth());
				if (null == dataResult.getValue1()) {
					dataResult.setValue1(new BigDecimal(0));
				}
				if (null == dataResult.getValue2()) {
		...
		data2.add(chartMap);
		data.put("data2", data2);
		data.put("seriesX", seriesX);
		return data;
	}

	@GetMapping(value = "findOrgRank")
	public Object findOrgRank(String month, UserInfo userInfo) {
		Map<String, Object> data = new HashMap<>();
		List<Map<String, Object>> data1 = Lists.newArrayList();
		List<Map<String, Object>> data2 = Lists.newArrayList();
		// 大横
		...
		QueryUtils.addWhere(qb, "and t.relationType = 'tag'");
		QueryUtils.addWhere(qb,
				" and not exists(from GroupRelation t1 where t1.tagType = '02' and t1.tagId = t.relationId)");
		QueryUtils.addWhere(qb,
				" and not exists(from IgnoreGroups t1 where t1.ignoreFlag = '1' and t1.tagid = t.relationId)");
		if (StringUtils.isNotBlank(month)) {
			QueryUtils.addWhereIfNotNull(qb, "and t.month = {0}", month);
		}
		QueryUtils.addOrder(qb, "t.total desc");
		...
		QueryUtils.addWhere(qb, "and t.relationType = 'tag'");
		QueryUtils.addWhere(qb,
				" and exists(from GroupRelation t1 where t1.tagType = '02' and t1.tagId = t.relationId)");
		QueryUtils.addWhere(qb,
				" and not exists(from IgnoreGroups t1 where t1.ignoreFlag = '1' and t1.tagid = t.relationId)");
		if (StringUtils.isNotBlank(month)) {
			QueryUtils.addWhereIfNotNull(qb, "and t.month = {0}", month);
	...
	public Object getDeptData(String timeYear, Integer deptId, UserInfo userInfo) {
		Map<String, Object> data = new HashMap<>();
		List<String>seriesX =Lists.newArrayList();
		...
		QueryUtils.addWhere(qb, "and t.relationId = {0}", deptId);
		// QueryUtils.addWhere(qb, "and t.month like {0}", "%" + timeYear +
		// "%");
		QueryUtils.addOrder(qb, "t.month");
		QueryUtils.addOrder(qb, "t.createDate desc");
